...
const VERBOSE_KEY = 'verboseNotifications';
const ESSENTIAL_EVENTS = new Set(['trade_entered', 'tp1_hit', 'tp2_hit', 'full_close', 'sl_hit', 'trade_closed']);
...
class TradingBot {
  ...
  _registerCommands() {
    ...
    this.bot.command(['setweeklylimit', 'setweeklytradelimit'], async (ctx) => {
      if (!this.tradeLimiter) {
        await ctx.reply('Weekly trade limit is not configured on this bot instance.');
        return;
      }
      ...
      if (!arg || !Number.isInteger(n) || n < 0) {
        await ctx.reply('Usage: /setweeklytradelimit <number>  e.g. /setweeklytradelimit 5');
        return;
      }
    ...
    this.bot.command('risk', async (ctx) => {
      const riskPct = await this.settingsStore.get('riskPct', null);
      const mode = await this.settingsStore.get('riskMode', 'per_venue');
      await ctx.reply(
        `⚖️ Risk sizing: ${riskPct ?? '(using deploy default)'}% per trade, mode: ${mode}\n` +
          (mode === 'per_venue'
            ? `Note: per_venue means each of your venues risks this % independently — with N venues mirroring one signal, real combined risk is roughly N × this %.`
            : `Note: total_portfolio splits this % risk budget evenly across however many venues are trading a given signal, so combined risk stays at roughly this %.`)
      );
    });

    this.bot.command('setrisk', async (ctx) => {
      const args = ctx.match?.trim().split(/\s+/) ?? [];
      const pct = Number(args[0]);
      const mode = args[1]?.toLowerCase();

      if (!args[0] || Number.isNaN(pct) || pct <= 0 || (mode && !['per_venue', 'total_portfolio'].includes(mode))) {
        await ctx.reply(
          'Usage: /setrisk <percent> [mode]\n' +
            'mode is "per_venue" (default, each venue risks this % independently) or "total_portfolio" (this % split across venues).\n' +
            'e.g. /setrisk 1 per_venue  or  /setrisk 2 total_portfolio'
        );
        return;
      }

      await this.settingsStore.set('riskPct', pct);
      if (mode) await this.settingsStore.set('riskMode', mode);
      const effectiveMode = mode ?? (await this.settingsStore.get('riskMode', 'per_venue'));
      await ctx.reply(`✅ Risk set to ${pct}% per trade, mode: ${effectiveMode}.`);
    });
    ...
    this.bot.command('verbose', async (ctx) => {
      const current = await this.settingsStore.get(VERBOSE_KEY, false);
      await this.settingsStore.set(VERBOSE_KEY, !current);
      await ctx.reply(!current ? '🔊 Verbose on: setup, zone and confluence messages will be sent.' : '🔇 Quiet mode: only trade entries, take-profits, stops and closes.');
    });
  ...
  async notifyFromEngine(setupManager, candles) {
    ...
    const verbose = await this.settingsStore.get(VERBOSE_KEY, false);

    for (const entry of newEntries) {
      const text = formatEngineEvent(entry);
      if (!text) continue;
      if (!verbose && !ESSENTIAL_EVENTS.has(entry.type)) continue;
