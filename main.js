...
function buildExecutionAdapters() {
  ...
  return {
    bybit: new CcxtAdapter('bybit', { apiKey: process.env.BYBIT_API_KEY, secret: process.env.BYBIT_SECRET }),
...
async function main() {
  ...
  if (!DRY_RUN && process.env.MT5_TOKEN && process.env.MT5_ACCOUNT_ID) {
